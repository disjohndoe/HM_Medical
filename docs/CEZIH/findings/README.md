...
| File | Date | Status | Summary |
|------|------|--------|---------|
| [2026-05-04-djelatnost-per-doctor-handoff.md](2026-05-04-djelatnost-per-doctor-handoff.md) | 2026-05-04 | active | **Dev handoff** for concern raised post-rejection refactor: djelatnost is hardcoded `1010000` "Opća medicina" everywhere - wrong for type 012 specialist nalaz. Spec'd User column + UI picker + threading through dispatcher/service/builder. Includes HZZO helpdesk question for test institution djelatnost. Not a cert blocker (validates) but ships placeholder data into signed clinical documents. |
| [2026-05-04-iti65-inner-fhir-document-bundle.md](2026-05-04-iti65-inner-fhir-document-bundle.md) | 2026-05-04 | active | **HZZO REJECTED 2026-05-04 10:59 (Natalija Malkoč) — invalidates 22/22 green sweeps + "PASSED 2026-05-04" memo.** Binary.data must be base64 of signed FHIR Document Bundle (Bundle.type=document, profile HRDocument), not plain text. Confirmed against `cezih.hr.klinicki-dokumenti#0.3` package: Composition profile `nalaz-iz-specijalisticke-ordinacije-privatne-ustanove` requires 2 authors (Practitioner+Org), 2 attesters, 2-3 sections (Djelatnost/Priloženi/Medicinska informacija with anamneza+slucaj+postupci+ishod sub-slices). Bundle.signature.who.reference is literal urn:uuid Practitioner. content.attachment.contentType must be `application/fhir+json`, Binary.contentType `application/json`. Affects TC18/19/20/22. Storno spec contradiction: vodič kaže entered-in-error, test env vraća ERR_DOM_10057 — re-verify after refactor. |
...
| [2026-05-18-multi-identifier-foreigner-inner-bundle-verified.md](2026-05-18-multi-identifier-foreigner-inner-bundle-verified.md) | 2026-05-18 | active | **Multi-slice Patient.identifier on inner Document Bundle verified GREEN at builder + wire level + exam-tenant cleanup.** Closes the last doc-gap on commit `3c605ce` (defensive fix from the 5-11 eKarton hint, not in any prior plan/finding). Reconciles "4 rejections" memory framing to the actual record: **2 emails, 4 issue items (1 on 5-04, 3 on 5-11), plus 1 defensive multi-slice fix** - all GREEN both signing methods, both patient classes. Added test EHIC to MOBILE STRANCERTI20260518; `resolve_all_cezih_identifiers()` returned 2 slices `[europska-kartica, putovnica]`; `_build_patient_resource()` emitted hr-pacijent with both `Patient.identifier[]` entries; signed inner Document Bundle on prod: 9 entries, 10903 bytes, sha256-prefix `b34a28999a25ad10`. CEZIH `ERR_DOM_10074` rejected the outer subject on the fake EHIC (boundary-validation working as designed). Post-test cleanup on prod DB: cleared fake EHIC on `e586318b`, deleted empty duplicate `1d165b44`, verified `TEST20251215113521HP` on HZZOTEST EHIC is CEZIH-accepted (1 successful visit today). Exam-tenant patient state matrix included in the finding doc - whichever patient Natalija picks on the next provjera, outer subject resolves to a CEZIH-accepted identifier. |
| [2026-05-18-mobile-certilia-three-rejection-items-verified.md](2026-05-18-mobile-certilia-three-rejection-items-verified.md) | 2026-05-18 | active | **Mobile Certilia leg of HZZO 2026-05-11 rejection items - all three GREEN.** Companion to the same-day smart-card sweep. Foreigner MOBILE STRANCERTI20260518 (passport `TEST20260518M`) PMIR `Patient/1611164`, CUID JID `cmpb1vvqa05lbhb85bf01qujp` refused by strict guard (cezih_id empty in DB), passport fallback for all wires. e-Nalaz chain **1611271 -> 1611285 -> Storniran** (POST/PUT/DELETE 200; 19.8s + 13.7s round-trips dominated by mobile push-to-approve). Full visit/case lifecycle on the same J06.9 case linked through `Encounter.diagnosis.condition.identifier=identifikator-slucaja`. ITI-67 e-Karton from CEZIH rendered 1 e-Nalaz row with doc-type display "Nalazi iz specijalisticke ordinacije privatne zdravstvene ustanove" (012). Zero ERR_* in 90min log window. Combined cert matrix now covers both signing methods x both patient classes; ready to request next provjera spremnosti termin. |
| [2026-05-18-pre-exam-three-rejection-items-verified.md](2026-05-18-pre-exam-three-rejection-items-verified.md) | 2026-05-18 | active | **All three HZZO 2026-05-11 rejection items verified GREEN on smart card, Croatian + foreigner.** Item 1 (foreigner JID format): CEZIH still returned CUID `cmpasvczt05k6hb85lou509yt` for HZZOTEST STRANAC20260518, JID strict guard refused to persist (cezih_patient_id NULL), every downstream FHIR op fell back to passport `TEST20260518A` via `resolve_cezih_identifier()`. Item 2 (doc-type x djelatnost): tenant 2030000 + user 2010000 (both prefix-2) emit HRTipDokumenta **012** for `specijalisticki_nalaz`/`nalaz`; six ITI-65 wires accepted (Croatian 1609343 -> 1609357 -> Storniran; foreigner **1609495 -> 1609504 -> Storniran**). Item 3 (posjeta x slucaj): `Encounter.diagnosis.condition.identifier` system=`identifikator-slucaja` renders as "Slucajevi J06.9 ..." on eKarton VisitsPlace row for both classes; DocumentReference carries both encounter+related. Companion to same-day Croatian-only sweep. Certilia mobile re-verify owed before requesting next provjera termin. |
| [2026-05-18-card-sweep-croatian-oib-pre-exam.md](2026-05-18-card-sweep-croatian-oib-pre-exam.md) | 2026-05-18 | active | **Pre-exam smart-card dry-run, Croatian GORAN OIB-keyed GREEN.** Visit `cmpaquahg05jyhb85lz45bbbk` full TC12/15/13/14 (1.1/1.2/1.3/1.4/1.5), case `cmpar5mbb05jzhb85my4vzwjk` (J06.9) full TC16 + TC17 2.3 -> 2.5 -> 2.4 -> 2.9 -> 2.6 (5x PUT 200, plus a second 2.6 retry after first dialog-close UX quirk), e-Nalaz Refs **1609281 -> 1609291 -> Storniran** (POST/PUT/DELETE 200; spare Ref 1609286 from accidental first submit). TC21 ITI-67 rendered 68 active dijagnoze. Zero ERR_* across 90min of backend logs. Wire op timings 11-44s. TC11/foreign + Certilia mobile still owed before next provjera attempt. |
| [2026-04-29-card-sweep-croatian-oib.md](2026-04-29-card-sweep-croatian-oib.md) | 2026-04-29 | active | **Smart-card sweep, Croatian GORAN OIB-keyed GREEN.** Mate to today's Certilia-mobile dual-patient sweep - card half of the dual-method exam matrix. Visit `cmojqiqgl03ykhb85v26fprfx` full TC12/15/13/14 (1.1/1.2/1.3/1.4/1.5), case `cmojqreau03ylhb85am3ab367` (J06.9) full TC16 + TC17 2.3 -> 2.5 -> 2.4 -> 2.9 -> 2.6 (5x PUT /status + /data 200), e-Nalaz Refs **1503283 -> 1503302 -> Storniran** (POST/PUT/DELETE 200). TC21 ITI-67 200 with 20 docs. Storno bundle inspected (status=current + relatesTo OID `urn:oid:2.16.840.1.113883.2.7.50.2.1.749991`, MBO subject path). Persistence verified via hard reload. Foreign-card leg pending. |
| [2026-04-29-card-sweep-foreign-ehic.md](2026-04-29-card-sweep-foreign-ehic.md) | 2026-04-29 | active | **Smart-card sweep, Foreign ROGER ROG EHIC-keyed GREEN.** Closes smart-card half of the dual-method, dual-patient exam matrix. Visit `cmojrtuy003yuhb85mi5fv9so` full TC12/15/13/14 (1.1/1.2/1.3/1.5/1.4), case `cmojs2oj203yvhb85k5mm1n2x` (J06.9) full TC16 + TC17 2.3 -> 2.5 -> 2.4 -> 2.9 -> 2.6 (5x PUT 200), e-Nalaz Refs **1504392 -> 1504559 -> Storniran** (POST/PUT/DELETE 200). TC21 ITI-67 200 with 20 docs. Storno bundle inspected (status=current + relatesTo OID `urn:oid:2.16.840.1.113883.2.7.50.2.1.750017`, **`jedinstveni-identifikator-pacijenta` subject path** confirming foreign branch). Persistence verified via hard reload. Together with today's Croatian-card + Croatian/foreign-mobile findings, all four method × patient-class quadrants now GREEN within the same day. |
| [2026-04-29-certilia-mobile-OIB-exam-dry-run-v2.md](2026-04-29-certilia-mobile-OIB-exam-dry-run-v2.md) | 2026-04-29 | active | **Third Certilia-mobile sweep of the day, Croatian GORAN OIB-only GREEN.** Reinforces mobile + OIB combo per "JUST DO mobile, for OIB" pivot. Visit `cmojugowz03z9hb85yyfgmops` full TC12/13/15/14, case `cmojushdm03zchb85kf3u7acy` (J06.9) full TC16 + TC17 2.3 -> 2.5 -> 2.4 -> 2.9 -> 2.6 (case round-trip in 3:15), e-Nalaz Refs **1505712 -> 1505716 -> Storniran** (doc OIDs `...750050`/`051`/`052`, POST/PUT/DELETE 200). TC21 ITI-67 200 (20 docs). Bearer-token fix (`960cf3e`) holds three sweeps deep in same day. Persistence verified via hard reload. |
| [2026-05-04-certilia-mobile-foreign-ehic-post-iti65-refactor-sweep.md](2026-05-04-certilia-mobile-foreign-ehic-post-iti65-refactor-sweep.md) | 2026-05-04 | active | **First foreign-EHIC Certilia-mobile sweep after ITI-65 inner Document Bundle refactor (`082ca4b`) GREEN.** ROGER ROG (UK, EHIC `TEST20251215113521HP`): slučaj `cmor9jdx004gnhb85cb4o2jqy` (J06.9) + posjeta `cmor9k6je04gohb85tva0alw2` + e-Nalaz **Ref 1532912** (List/1532911, Binary/1532913, doc OID `...751478`), three POSTs 200 in one Certilia approval session. Inner Bundle.type=document with Composition type=012, foreign subject path `jedinstveni-identifikator-pacijenta` everywhere (no MBO). Closes foreign-mobile gap left by earlier-today Croatian-only post-refactor run. TC19/TC20 round-trip + smart-card foreign sweep still owed. |
| [2026-05-04-certilia-mobile-OIB-post-iti65-refactor-sweep.md](2026-05-04-certilia-mobile-OIB-post-iti65-refactor-sweep.md) | 2026-05-04 | active | **First Certilia-mobile OIB sweep after ITI-65 inner Document Bundle refactor (`082ca4b`) GREEN.** Confirms post-HZZO-rejection refactor (signed FHIR Document Bundle inside Binary, not plain text) is accepted by CEZIH on TC18/19/20 via Certilia mobile. Croatian GORAN: case `cmor764y604gdhb85eewfeln2` (J06.9) full TC16 + TC17 lifecycle, e-Nalaz Refs **1532426 -> 1532446 -> Storniran** (doc OIDs `...751468`/`751469`/`751470`, POST/PUT/DELETE 200). TC21 ITI-67 200 (20 docs, 4.1s). One transient VPN flap mid-run between TC20 and TC21, agent auto-reconnected ~60s, OAuth backoff held, retry 200. Persistence verified via hard reload. |
| [2026-05-05-meta-profile-sweep-visits-cases-pmir.md](2026-05-05-meta-profile-sweep-visits-cases-pmir.md) | 2026-05-05 | active | **Audit-driven meta.profile sweep across visits (TC1.1/1.2), cases (2.1/2.2/2.3-2.5/2.6/2.9), and PMIR (TC11) - same risk pattern as 2026-05-04 ITI-65 rejection.** Documents path was fixed via 082ca4b/9211d1a/bbb2b2e but visits, cases, and PMIR were unchanged. Audit found visits 1.1/1.2 passing `profile_urls=None`, every case lifecycle event missing profile_urls entirely (no `CASE_EVENT_PROFILE_MAP` existed), and PMIR inner Bundle + Patient missing meta.profile with explicit rationalizing comment. Fix: added `CASE_EVENT_PROFILE_MAP` mirroring `ENCOUNTER_EVENT_PROFILE_MAP`, wired profile_urls (bundle+header+resource) into all four case call sites and both visit create/update sites, added meta.profile on PMIR inner Bundle (`IHE.PMIR.Bundle.History`) and Patient (`hr-pacijent`). 22 TC matrix re-verification on smartcard + Certilia required before next provjera termin. Deferred MED items: signature.who divergence, SVCM `$expand` silent fallback, OID provenance gap. |
| [2026-05-05-signature-who-message-bundle-audit.md](2026-05-05-signature-who-message-bundle-audit.md) | 2026-05-05 | resolved | **Audit MED #4 closed by inheritance walk: identifier-based `signature.who` is correct on all message-bundle paths.** Downloaded `cezih.hr.encounter-management/0.2.3` + `cezih.hr.condition-management/0.2.1`; all 7 visit + 7 case per-event profiles inherit `hr-request-message`, which leaves `signature.who` as a plain `Reference` with no slicing. PMIR `HRPMIRBundle` is the only path with explicit slicing and it REQUIRES identifier shape (mandates `Practitioner` type + HZJZ-broj-zdravstvenog-djelatnika system patternUri). Our `practitioner_ref()` matches exactly, no `signing.py` rewrite needed. HRDocument urn:uuid constraint (DOC-3) is documents-only, not a precedent. Side discovery: Stage 0 audit fix mapped `CASE_EVENT_PROFILE_MAP[2.6]` to non-existent `hr-update-health-issue-data-message` (typo from `case-lifecycle-profile-matrix.md`); correct profile is `hr-health-issue-update-message` per IG package event coding. Inline fix applied to `condition.py` in same commit. |
| [2026-05-04-meta-profile-on-inner-bundle-resources-gap1-green.md](2026-05-04-meta-profile-on-inner-bundle-resources-gap1-green.md) | 2026-05-04 | active | **Gap 1 (meta.profile on inner Document Bundle resources) deployed and accepted by CEZIH first try, GREEN.** Commit `9211d1a` adds `meta.profile` declarations on all 10 inner-bundle resources (Bundle hr-document, Composition per 011/012/013, Patient hr-pacijent, Practitioner hr-practitioner, Organization hr-organizacija, Encounter hr-encounter, Condition dokumentirani-slucaj, anamneza, ishod-pregleda, djelatnost). ROGER ROG foreign-mobile sweep: slučaj `cmorajjn104gshb854ge9fsd6` + posjeta `cmorak6au04gthb85m5b3zben` + e-Nalaz **Ref 1533184** (List/1533183, Binary/1533185, doc OID `...751508`), POST 200 in 21.2s with 3x `201 Created`. Signed bundle grew 9763 -> 10649 bytes (+886 for the meta.profile arrays). Closes most-likely-remaining cert risk flagged by 2026-05-04 slice-by-slice audit against `cezih.hr.klinicki-dokumenti` Simplifier package. |
| [2026-05-05-certilia-mobile-post-meta-profile-audit-sweep.md](2026-05-05-certilia-mobile-post-meta-profile-audit-sweep.md) | 2026-05-05 | active | **Post-meta.profile-audit Certilia-mobile sweep - TC12-TC21 all GREEN.** First complete sweep after 2026-05-05 meta.profile audit commits (f305557 + 688de14 + bbb2b2e + 668857f + 9211d1a). Croatian GORAN OIB dry-run: visit TC12/13/15/14 (visit `cmosqvtqs04kghb85tdnh5mlq` cancelled), case `cmosr0iir04kkhb85hzsbig1p` (J06.9) TC16 + TC17 path 2.3->2.5->2.9->2.6 (2.4 deferred - verified in prior sessions, requires Remisija state). e-Nalaz Refs **1540768 -> 1540807 -> Storniran** (TC18 POST 200, TC19 PUT 200, TC20 DELETE 200 14.1s - storno completed within same Certilia session as TC19). TC21 ITI-67 200 (20 docs, 57 diagnoses, 22 visits, 39 e-Nalaz history). System ready for next HZZO provjera termin. |

...
| [2026-04-21-cezih-2.2-recurrence-not-supported.md](2026-04-21-cezih-2.2-recurrence-not-supported.md) | 2026-04-21 | resolved | **RESOLVED via H2a** (commit 90ab916) — kept the `lokalni-identifikator` slice instead of stripping entire identifier array. CEZIH HTTP 200. Follow-up: parser doesn't persist new global ID locally; FE button still hidden until that's fixed. |
...
| [2026-04-21-handoff-2.2-and-2.6-profile-fix.md](2026-04-21-handoff-2.2-and-2.6-profile-fix.md) | 2026-04-21 | partial | Handoff: 2.6 resolved via H1 (drop asserter). 2.2 Ponavljajući still pending — next apply same delta to `build_condition_create` + retest; investigate `vezani-slucaj` extension for parent linkage if that also fails. |

...
OID prefix:   2.16.840.1.113883.2.7.50.2.1.{suffix}  (auto-generated by TC6 API, NOT from HZZO)
```
...
|----|--------|-------------|-----------------|
| TC6 | Generiraj OID | CEZIH → Registri → OID | OID `2.16.840.1.113883.2.7.50.2.1.{suffix}` (FHIR system OID, auto-generated — NOT institution identifier) |
...
### TC22 — CEZIH Test Documents Have No Binary Content
ALL documents in CEZIH test environment return 0 bytes (empty Binary). DocumentReference records exist but the actual binary content is not stored. During the exam, retrieve documents created DURING the session (TC18/TC19) — CEZIH may retain newly uploaded documents in memory. Our system correctly handles empty responses by wrapping them as PDF (43KB placeholder). The ITI-68 flow itself is verified working — binary transport through agent, base64 decoding, and PDF response all function correctly.

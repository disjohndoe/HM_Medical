"""ISO 3166-1 alpha-2 → alpha-3 conversion for CEZIH PMIR.

CEZIH HRRegisterPatient profile binds address.country to ValueSet/drzave
which uses alpha-3 codes. Frontend forms often hold alpha-2 — use
to_alpha3() to normalize.
"""
...
def to_alpha3(raw: str) -> str:
    """Normalize a country code to ISO 3166-1 alpha-3.

    2-char input is looked up; 3-char input is returned as-is; anything
    else is returned unchanged (let CEZIH reject it rather than guess).
    """
    raw = (raw or "HRV").upper().strip()
    if len(raw) == 2:
        return _ALPHA2_TO_ALPHA3.get(raw, raw)
    return raw
